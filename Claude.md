...
-   **Focus on the MVP:** Prioritize the core features: code ingestion, AI-powered documentation generation, and a Q&A API. Features like complex knowledge graph visualizations or project management integrations are for later iterations.

## 2. Project Structure

Organize the codebase to maintain a clear separation of concerns.
...
-   **AI Service Interface:** Define an interface for all interactions with LLMs in `internal/ai/ai.go`. The initial implementation will use the OpenAI API, but this interface allows for future expansion to other models (local or cloud-based).
...
-   **Retrieval-Augmented Generation (RAG):** The Q&A logic resides in `internal/qa`.
    1.  **Chunking:** Code from `internal/analysis` will be chunked into logical pieces (e.g., by function).
    2.  **Embedding:** Use the `ai.Service` to generate embeddings for each chunk.
    3.  **Storage & Retrieval:** For the MVP, store vectors in a simple in-memory slice or map within the `store` package. Implement a brute-force cosine similarity search for retrieval. This is sufficient for the initial scope.
    4.  **Prompting:** Construct a clear prompt for the LLM that includes the user's question and the retrieved code chunks as context.

## 5. CLI Tool

-   Use the `cobra` library to build the CLI in `cmd/cli/`.
-   For the MVP, the CLI acts as a thin client. It makes HTTP requests to the running backend service. Do not duplicate business logic from the `internal` packages in the CLI.

## 6. Testing

-   Write unit tests for core logic in `analysis`, `qa`, and `ai`.
